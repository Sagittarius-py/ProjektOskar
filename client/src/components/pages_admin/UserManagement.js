...
const UserManagement = () => {
  ...
  const [userDataInEdit, setUserDataInEdit] = useState({
    email: "",
    password: "",
    address: "",
  });
  ...
  useEffect(() => {
    Axios.get("http://localhost:8000/api/getAllUsers").then((data) =>
      setUsers(data.data)
    );
  }, []);
  ...
  const handleSubmitEditedUser = async (userId) => {
